...
class UserServer(BaseModel, HttpSessionMixin):
  ...
  async def init_server(self, user):
    ...
    try:
      ...
      udp_server, _ = await loop.create_datagram_endpoint(LocalUDP(user), (self.HOST, user.port))
      ...
      self.save()
      logging.info("user:{} method:{} password:{} port:{} 已启动".format(user, user.method, user.password, user.port))
    except OSError as e:
      logging.warning(e)
  ...
  def check_traffic_rate(self, data_lens):
    if self.traffic_limiter.consume(data_lens):
      logging.warning(f"reach traffic limit: {self.traffic_limiter.human_rate} user_id: {self.user_id}")
      return True
    return False
